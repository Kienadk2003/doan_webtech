...
class ProductsController extends Controller
{
    ...
    {
    // Sắp xếp sản phẩm theo id tăng dần rồi đảo ngược để nhập lâu nhất ở dưới cùng
    $products = Products::orderBy('id', 'ASC')->get()->reverse()->values();
    ...
    public function edit(Request $request,$id)
    {
        ...
        $products->update();
        return redirect('/admin/accessories')->with('toast_success',__("Update successfully"));
    }
    ...
    // Filter sold quantities by month or date range. Returns JSON when requested (ajax=1 or X-Requested-With)
    public function filterSold(Request $request){
        ...
        $soldQuery = DB::table('orders__details as od')
            ->join('orders as o','od.orders_id','=','o.id')
            ->where('o.status', 3)
            ->select('od.products_id', DB::raw('SUM(od.quantity) as sold_qty'));

        if($start && $end){
            $soldQuery->whereBetween('o.created_at', [$start->toDateTimeString(), $end->toDateTimeString()]);
        }

        $soldRows = $soldQuery->groupBy('od.products_id')->get();
        $sold_map = [];
        foreach($soldRows as $r){
            $sold_map[$r->products_id] = (int)$r->sold_qty;
        }

        // If a time filter is applied, show only products that have sold > 0 in that period
        $products = null;
        $timeFiltered = ($start && $end);
        if($timeFiltered && count($sold_map) > 0){
            $ids = array_keys($sold_map);
            $products = Products::whereIn('id', $ids)->get()->sortByDesc(function($p) use ($sold_map){
                return $sold_map[$p->id] ?? 0;
            })->values();
        }else{
        ...
        $rows_html = view('admin.pages.products._sold_rows', [
            'products' => $products,
            'sold_map' => $sold_map
        ])->render();
    ...
    // Simple debug endpoint to inspect sold aggregation
    public function debugFilter(Request $request){
        $soldRows = DB::table('orders__details as od')
            ->join('orders as o','od.orders_id','=','o.id')
            ->where('o.status',3)
            ->select('od.products_id', DB::raw('SUM(od.quantity) as sold_qty'))
            ->groupBy('od.products_id')
            ->get();
        return response()->json(['ok' => true, 'rows' => $soldRows]);
    }
}
