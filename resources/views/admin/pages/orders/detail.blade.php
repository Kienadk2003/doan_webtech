        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="order_detail">{{ __("Order Number") }}: {{ $order->id }} -
                    Ngày nhận: {{ $order->updated_at->format('d/m/Y') }}
                    <a href="javascript:void(0)" style="color:white" class="btn bg-info" onclick="status({{ $order->id }},2)">Xác nhận</a>
                </h5>
                ...
                <table class='table table-bordered text-nowrap'>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td colspan="4" class="text-wrap">{{ number_format($order->subtotal, 0, ',', '.') }}<sup style="text-decoration: underline; padding: 3px; text-transform: lowercase !important;">VND</sup></td>
                        </tr>
                        <tr>
                            ...
                            <td colspan="4" class="text-wrap">{{ number_format($order->tax, 0, ',', '.') }}<sup style="text-decoration: underline; padding: 3px; text-transform: lowercase !important;">VND</sup></td>
                        </tr>
                        <tr>
                            <th class="text-center" colspan="2" style="width:5%">{{ __("Discount") }} ({{ (($order->subtotal - ($order->total - $order->tax)) / $order->subtotal) * 100 }}%)</th>
                            <td colspan="4" class="text-wrap">{{ number_format($order->discount, 0, ',', '.') }}<sup style="text-decoration: underline; padding: 3px; text-transform: lowercase !important;">VND</sup></td>
                        </tr>
                        <tr>
                            <th class="text-center" colspan="2" style="width:5%">{{ __("Total") }}</th>
                            <td colspan="4" class="text-wrap">{{ number_format($order->total, 0, ',', '.') }}<sup style="text-decoration: underline; padding: 3px; text-transform: lowercase !important;">VND</sup></td>
                        </tr>
                        ...
                        <thead>
                            ...
                            <tr>
                                <th colspan="2" class="text-center">Họ người gửi</th>
                                <th colspan="2" class="text-center">Tên người gửi</th>
                                <th colspan="2" class="text-center">Số điện thoại người gửi</th>
                            </tr>
                        </thead>
                    <tbody>
                        <tr>
