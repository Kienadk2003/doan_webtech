                ...
                <table class='table table-striped text-nowrap' id="table1">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">{{ __("Delivery in Progress") }}</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <a href="javascript:void(0)" onclick="if(confirm('Đơn hàng đã được thanh toán chưa?')){status({{ $order->id }},2);}">
                                    <i data-feather="truck" stroke="blue"></i>
                                </a>
                            </td>
                            <td class="text-center">
                                <a href="javascript:void(0)" onclick="if(confirm('Đơn hàng đã được giao thành công.')){status({{ $order->id }},3);}">
                                    <i data-feather="check" stroke="green"></i>
                                </a>
                            </td>
                            <td class="text-center">
                                <a href="javascript:void(0)" onclick="if(confirm('Bạn có muốn hủy đơn hàng này không?')){status({{ $order->id }},4);}">
...
$(function () {
    ...
    $('#orders_filter').on('click', function (e) {
        e.preventDefault();
        applyOrdersFilter();
    });

    $('#orders_reset').on('click', function (e) {
        e.preventDefault();
        $('#orders_start').val('');
        $('#orders_end').val('');
        $('#orders_month').val('');
    ...
    $('#orders_export').on('click', function (e) {
        ...
        if (month) {
            ...
            if (start) url += 'start=' + encodeURIComponent(start);
            if (end) url += (url.endsWith('?') ? 'end=' : '&end=') + encodeURIComponent(end);
        }
