		...
		<div class="row justify-content-center">
			...
			<div class="col-xl-6 col-lg-6 col-md-8">
				<div class="box_account">
					...
					<div id="signup_success_msg" class="alert alert-success" style="display:none; margin-bottom:15px;">
						<strong>{{__('Success')}}!</strong> <span id="signup_msg_text"></span>
					</div>
					<div class="form_container">
						...
						<div class="form-group">
							<div class="password-wrapper">
								<input type="password" class="form-control" name="password" id="password_signup" value="" placeholder="{{__('Password')}}*">
								<i class="fa fa-eye" id="toggleSignupPassword"></i>
							</div>
						</div>
						<div class="form-group">
							<div class="password-wrapper">
								<input type="password" class="form-control" name="repassword" id="repassword_signup" value="" placeholder="{{__('Confirm Password')}}*">
								<i class="fa fa-eye" id="toggleSignupRePassword"></i>
							</div>
						</div>
							<hr>
							...
							<hr>
							<div class="text-center"><input type="submit"class="btn_1 full-width" value="{{__('Sign Up')}}"></div>
						</form>
					</div>
					<!-- /form_container -->
				</div>
...
<script>
   $(document).ready(function() {
	   // Sắp xếp option city theo a-z
	   var $city = $('#city');
	   var $options = $city.find('option');
	   var $first = $options.first();
	   var sorted = $options.slice(1).sort(function(a, b) {
		   return $(a).text().localeCompare($(b).text(), 'vi', {sensitivity: 'base'});
	   });
	   $city.empty().append($first).append(sorted);
	   $city.select2();
	   ...
	   $('#toggleSignupPassword').click(function() {
		   var passwordField = $('#password_signup');
		   var type = passwordField.attr('type') === 'password' ? 'text' : 'password';
		   passwordField.attr('type', type);
		   $(this).toggleClass('fa-eye fa-eye-slash');
	   });

	   $('#toggleSignupRePassword').click(function() {
		   var repasswordField = $('#repassword_signup');
		   var type = repasswordField.attr('type') === 'password' ? 'text' : 'password';
		   repasswordField.attr('type', type);
		   $(this).toggleClass('fa-eye fa-eye-slash');
	   });
	   ...
	   $('#signup_form').on('submit', function(e) {
		   ...
		   var $successMsg = $('#signup_success_msg');
		   var $msgText = $('#signup_msg_text');
		   ...
		   $.ajax({
			   ...
			   success: function(resp) {
				   var msg = (resp && resp.message) ? resp.message : '{{__('Sign Up Successfully')}}';
				   $msgText.text(msg);
				   $successMsg.fadeIn();
				   $form[0].reset();
				   $city.val('').trigger('change');
				   $('html, body').animate({ scrollTop: $successMsg.offset().top - 50 }, 300);
			   ...
			   error: function(xhr) {
				   var msg = 'Đăng ký không thành công. Vui lòng kiểm tra lại thông tin.';
